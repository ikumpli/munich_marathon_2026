...
# How many of the most recent runs get HR/pace curve data (matches the
# "Recent Sessions" table on the dashboard).
N_CURVE_RUNS = 10
CURVE_MAX_POINTS = 150

ROOT = Path(__file__).resolve().parents[1]
CSV_PATH = ROOT / "i600311_activities.csv"
WELLNESS_CSV_PATH = ROOT / "i600311_wellness.csv"
...
def fetch_wellness(api_key: str, oldest: str = "2024-01-01") -> list:
    """Fetch daily wellness records (includes CTL/ATL computed by Intervals.icu)."""
    response = requests.get(
        WELLNESS_URL,
...
def wellness_to_df(records: list) -> pd.DataFrame:
    """Map Intervals.icu wellness records to a date/ctl/atl/form table.
    Form (a.k.a TSB) isn't always present in the trimmed `fields` response,
    so it's always derived here rather than trusted from the API."""
    rows = []
    for rec in records:
        ctl = rec.get("ctl")
        atl = rec.get("atl")
...
def fetch_streams(api_key: str, activity_id: str) -> dict:
    """Fetch time-series streams for one activity. Returns {} if unavailable."""
    response = requests.get(
    ...
    response.raise_for_status()
    raw = response.json()
    # Intervals.icu has been observed returning either a list of
    # {"type": ..., "data": [...]}  or a dict already keyed by stream type.
...
def _downsample(*series):
    """Stride-sample parallel arrays down to CURVE_MAX_POINTS, keeping alignment."""
    n = len(series[0])
    if n == 0:
        return series
...
def build_run_curves(runs_df: pd.DataFrame, api_key: str) -> dict:
    """Fetch HR/pace streams for the most recent N_CURVE_RUNS runs."""
